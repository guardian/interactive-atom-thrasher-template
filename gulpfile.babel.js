...
const isDeploy =
  gutil.env._.indexOf("deploylive") > -1 ||
  gutil.env._.indexOf("deploypreview") > -1;
const live = gutil.env._.indexOf("deploylive") > -1;

const version = `v/${Date.now()}`;
const s3Path = `atoms/${config.path}`;
const assetPath = isDeploy
  ? `${cdnUrl}/${s3Path}/assets/${version}`
  : "../assets";
...
const local = () => {
  ...
  const atomPromises = atoms.map((atom) => {
    ...
    const css = _template(
      fs.readFileSync(`.build/${atom}/main.css`).toString()
    );
    const html = _template(
      fs.readFileSync(`.build/${atom}/main.html`).toString()
    );
...
const awsCredentials = new AWS.CredentialProviderChain([
  ...
  function () {
    return new AWS.SharedIniFileCredentials({ profile: "interactives" });
  },
]);
...
const upload = () => {
  ...
  const uploadTasks = atoms.map((atom) => {
    ...
    return src(`.build/${atom}/*`)
      ...
      .pipe(replace("&lt;%= path %&gt;", assetPath))
      .pipe(
        replace("<%= atomPath %>", `${cdnUrl}/${s3Path}/${atom}/${version}`)
      )
  ...
  uploadTasks.push(
    src(`.build/assets/**/*`).pipe(
      s3Upload("max-age=31536000", `${s3Path}/assets/${version}`)
    )
  );
...
const getAtoms = () =>
  fs.readdirSync(".build").filter((n) => n !== "assets" && n !== "index.html");
...
const url = (cb) => {
  ...
  atoms.forEach((atom) => {
    ...
    gutil.log(
      gutil.colors.yellow(
        `https://content.guardianapis.com/atom/interactive/interactives/${config.path}/${atom}`
      )
    );
  });
...
const getLogs = async (cb) => {
  ...
  for (let i = 0; i < atoms.length; i++) {
    const atom = atoms[i];
    const liveLog = await rp(
      `${cdnUrl}/atoms/${config.path}/${atom}/live.log`
    ).catch((err) => console.log(`LIVE REQUEST ERROR: ${atom}`));
    const previewLog = await rp(
      `${cdnUrl}/atoms/${config.path}/${atom}/preview.log`
    ).catch((err) => console.log(`PREVIEW REQUEST ERROR: ${atom}`));
